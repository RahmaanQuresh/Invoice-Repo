...
export async function POST(
  ...
  try {
    ...
    // Resume the reminder sequence
    await prisma.invoice.update({
      where: { id: reminder.invoice.id },
      data: { reminderPaused: false },
    });

    return NextResponse.json({ success: true, data: { paused: false } });
  } catch (error) {
    console.error("Error resuming reminders:", error);
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Failed to resume reminders" } },
      { status: 500 }
    );
  }
}
